...
def main():
    ...
    #加载数据
    ji=r'/home/jishengpeng/NlpVoice/Data/DiffBeautifier/chaplin晚安.wav'
    ...
    plt.savefig("/home/jishengpeng/NlpVoice/DiffBeautifer/result/image1.png")
    plt.close()
    ...
    ## 初始化损失函数
    loss_f = torch.nn.MSELoss().to(device) ## |X-Y| ## 可以换成 torch.nn.MSELoss()
    ...
    for traini in range(args.epoch):
        ...
        for j in range(args.train_epoch):
            ...
            if(traini%10==0):
                ...
                write_line2log(loss_d,"/home/jishengpeng/NlpVoice/DiffBeautifer/result/train_log.txt",True)
        ...
        if(traini%10==0):
            ...
            plt.title(f'Mel-Spectrogram')
            plt.colorbar(mel_img,format='%+2.0f dB')
            plt.savefig("/home/jishengpeng/NlpVoice/DiffBeautifer/result/testresult/imageepoch"+str(traini)+".png")
            plt.close()

            #合成一些语音听一听
            wav1 = melspectrogram2wav(tmp.T) # input size : (frames ,ndim)
            sr=16000
            outputfile="/home/jishengpeng/NlpVoice/DiffBeautifer/result/testresult/stdioepoch"+str(traini)+".wav"
